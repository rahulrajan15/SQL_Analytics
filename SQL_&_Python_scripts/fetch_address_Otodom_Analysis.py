...
geolocator = Nominatim(user_agent="otodomprojectanalysis_set1")
    
engine = create_engine(URL(
                    account = 'nvpxgsu-xzb32165',
                    user = 'kent18',
                    password = 'Kent#$*',
                    database = 'otodom_database',
                    schema = 'public',
                    warehouse = 'otodom_wh'))
    ...
    try:
        query = """ SELECT RN, concat(latitude,',',longitude) as LOCATION
                    FROM (SELECT RN
                        ...
                        FROM otodom_data_short_flatten WHERE rn between 1 and 50
                        ORDER BY rn  ) """
